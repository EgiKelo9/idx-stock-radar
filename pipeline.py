...
class SignalPipeline:
    ...
    def process_ticker(self, ticker: Ticker) -> Optional[SignalPayload]:
        """
        Executes multi-layer screening and risk evaluation for a single ticker.
        ...
        # Layer 1: Volume Spike Filter
        has_spike, vol_mult, spike_reason = check_volume_spike(
            current_volume=int(current_volume),
            avg_volume_20d=avg_vol_20d,
            current_turnover=turnover,
            multiplier_threshold=config.risk.volume_spike_threshold,
        ...
        setup_type, metrics = detect_technical_setup(df_indicators, vol_mult)
        if setup_type is None:
            logger.debug(f"[{symbol}] No valid technical setup detected")
            return None
        ...
        # Layer 3: Broker Flow (Bandarmologi)
        broker_summary = self.broker_fetcher.fetch_broker_summary(symbol)
        ...
        dispatched = self.notifier.dispatch_signal(signal)
        if dispatched:
            logger.info(f"Signal dispatched for {symbol} ({setup_type})")
